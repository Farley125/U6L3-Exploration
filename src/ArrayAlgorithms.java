public class ArrayAlgorithms {
   ...
   */
    ...
    public static int[] squares(int[] numList) {
        ...
        for (int i = 0; i < numList.length; i++) {
            newArray[i] = (int) Math.pow(numList[i], 2);
        }
        return newArray;
    }
    ...
    public static String[] makeLowercase(String[] wordList) {
        String[] output = new String[wordList.length];
        for (int i = 0; i < wordList.length; i++) {
    ...
    public static boolean containsString(String[] stringList, String searchStr) {
        for (String string : stringList) {
            if (string.toLowerCase().indexOf(searchStr.toLowerCase()) != -1) {
                return true;
            }
        }
        return false;
    ...
    public static int[] arrayMaximums(int[] intArr1, int[] intArr2) {
        int[] output = new int[intArr1.length];
        for (int i = 0; i < intArr1.length; i++) {
            if (intArr1[i] > intArr2[i]) {
                output[i] = intArr1[i];
            } else output[i] = intArr2[i];
        }
        return output;
    }
    ...
    public static int[] shiftLeft(int[] numList) {
        int firstDig = numList[0];
        int[] output = new int[numList.length];
        for (int i = 0; i < numList.length; i++) {
            if (i == numList.length-1) {
                output[i] = firstDig;
            } else output[i] = numList[i+1];
        }
        return output;
    }
